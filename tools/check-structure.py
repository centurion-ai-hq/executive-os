...
drills = [f for f in R.glob("plugins/*/skills/*/SKILL.md")
          if re.search(r'^description:\s*["\']?Full sequence\.', f.read_text(), re.M)]
say("5 full sequences", len(drills) == 5, str(len(drills)))
bad, notrig, toolong = [], [], []
...
# Every skill must be DISTINGUISHABLE in the injected roster. A line like "Full sequence." tells
...
PAIRS = [("document", "sop"), ("brain-dump", "roadmap"), ("decide", "priorities"),
         ("critic", "pressure-test"), ("prep", "stakeholder-brief"), ("process-audit", "incident"),

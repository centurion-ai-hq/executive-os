...
```
  ...
  delegate-to-agents: Full sequence.
...
entries (`pressure-test`, `check-numbers`, `delegate-to-agents`, `lesson-learned`, `prove-it`) show only `Full sequence.`
...
| 28 | find out what other veteran-serving organisations are actually paying for this kind of software. | research | research | yes |
...
| 65 | build me a list of mission-driven organisations in Arizona doing workforce development, the kind that'd actually buy this. | prospects | prospects | yes |
...
| 108 | what federal or state money is on the table for mission-driven organisations like Relief Corps. | grants | grants | yes |
...
| 116 | I need the numbers on how many veterans we actually helped, something I can show funders. | impact-report | impact-report | yes |
...
1. **The 5 blank-roster skills were never tested.** `pressure-test`, `check-numbers`, `delegate-to-agents`,
   `lesson-learned`, `prove-it` all read `Full sequence.` in the injected roster with no other content.
   A model choosing between them and a neighbor (e.g. "poke holes in this" could plausibly
   go to `pressure-test` instead of `critic`, or "prove-it it" has literally nothing in the roster to
   ...
   the standalone sentence `"Full sequence."`, so `one_liner()` returns exactly that and
...
2. **Even the fully-described skills were never tested against their nearest neighbor in
   ...
   - `critic` vs `pressure-test` (untested, but real): both trigger on "tear this apart" /
     "before I commit" language. `critic.md`'s own description literally shares the phrase
     "before they commit to it" with `pressure-test.md`'s "before you commit to it."
...
1. **`pressure-test`, `check-numbers`, `delegate-to-agents`, `lesson-learned`, `prove-it`** (roster shows `Battle
   ...
   is to stop opening the description with a bare "Full sequence." sentence and fold the
   differentiator into that first sentence instead. Concrete rewording for each first
   sentence (drop the standalone "Full sequence." lead-in):
   - `pressure-test`: "Attacks a plan from four angles (facts, strongest failure case, over-scope, cost-vs-return) before you commit to it."
   - `check-numbers`: "Re-derives every number in a document back to its original source before it drives a decision."
   - `delegate-to-agents`: "Hands a whole multi-part job to the agent team instead of doing it in one pass, and returns the finished work."
   - `lesson-learned`: "Turns a correction into something that catches itself next time, logged to the corrections file."
   - `prove-it`: "Verifies a claim before you act on it and returns PROVED, DISPROVED, or UNPROVEN with evidence attached."
...
3. **`critic` vs `pressure-test`**, both currently open on near-identical language ("before they
   ...
   single-voice version of `pressure-test`'s four-angle full sequence." Naming the sibling skill
...
## Methodology note on what this run does and doesn't prove-it
...
The one_liner() truncation bug is very likely costing real accuracy on `pressure-test`,
`check-numbers`, `delegate-to-agents`, `lesson-learned`, and `prove-it` in actual use; that is invisible to this

...
- **First 20 lines (five untested skills: pressure-test, prove-it, check-numbers, lesson-learned, delegate-to-agents): 20/20 = 100%**
...
| Pair | Lines | Score |
|---|---|---|
| pressure-test vs critic | 21–24 | 4/4 |
...
|---|---|---|---|---|
| 1 | Before I sign off on this budget, I want someone to poke holes in it from every angle - facts, failure case, whether it's overbuilt, whether the math actually works. | pressure-test | pressure-test | MATCH |
| 2 | We're about to approve the Q4 expansion plan and I need it stress tested hard before Friday's board call. | pressure-test | pressure-test | MATCH |
| 3 | Can you tear into this strategy doc before I bring it to the partners? I want the worst-case argument laid out too. | pressure-test | pressure-test | MATCH |
| 4 | This budget looks clean but I want it hit from every side - is it too much, are the numbers real, what's the case it flops. | pressure-test | pressure-test | MATCH |
| 5 | Marketing keeps saying the campaign lifted signups 40 percent and I want that checked against the actual data before we brag about it in the board deck. | prove-it | prove-it | MATCH |
| 6 | You told me the migration was done last week - I need that confirmed against something real before I tell the client it's live. | prove-it | prove-it | MATCH |
| 7 | Our ops lead swears the new vendor cut costs 15 percent, but I want that run down against the invoices before I repeat it anywhere. | prove-it | prove-it | MATCH |
| 8 | The report says churn dropped this quarter. Before I put that in front of investors, somebody needs to trace it back to the source. | prove-it | prove-it | MATCH |
...
| 13 | This is the third time you've quoted a price we froze months ago - I need that to actually stop happening, not just get apologized for again. | lesson-learned | lesson-learned | MATCH |
| 14 | We've been over this before about sending drafts without asking first, and it keeps slipping through, so make it stick this time. | lesson-learned | lesson-learned | MATCH |
| 15 | I corrected this same formatting mistake two weeks ago and here it is again - I need a real fix, not another sorry. | lesson-learned | lesson-learned | MATCH |
| 16 | You keep mixing up which account this data belongs to, and I've flagged it before. Make sure it actually holds this time. | lesson-learned | lesson-learned | MATCH |
| 17 | This competitor teardown is way too big for one pass - get some people digging, someone drafting, and someone else checking it before it lands on my desk. | delegate-to-agents | delegate-to-agents | MATCH |
| 18 | I need the vendor comparison researched, written up, and reviewed by a second set of eyes, not just knocked out in one go. | delegate-to-agents | delegate-to-agents | MATCH |
| 19 | Split the onboarding rewrite across a few hands - research the gaps, draft the fix, then have someone else check it before it comes back to me. | delegate-to-agents | delegate-to-agents | MATCH |
| 20 | This is too much for one shot. Get it researched, drafted, and reviewed in stages instead of trying to answer it all at once. | delegate-to-agents | delegate-to-agents | MATCH |
...
| 23 | Before the board votes on this budget Friday, run it through the full gauntlet - facts, failure case, bloat, and whether the money actually works. | pressure-test | pressure-test | MATCH |
| 24 | This restructuring plan needs the whole workup before I approve it: check the assumptions, find the failure mode, see if it's overbuilt, and stress the numbers. | pressure-test | pressure-test | MATCH |
...
**1. pressure-test vs critic, sufficient, strong.**
- `pressure-test: Full sequence. The full four-stage review of a plan the user is about to approve: the facts, the case tha[t...]`
...
## What this test cannot prove-it about production
...
4. The five "previously untested" skills arrived in their own clean four-line block, immediately adjacent to their own vocabulary. Nothing here tests whether pressure-test/prove-it/check-numbers/lesson-learned/delegate-to-agents stay correctly separated from each other or from older skills when a real sentence doesn't announce its category so cleanly.
...
---
**Word count: this analysis section is agent-facing, not an executive-facing document; the 500/1500-word
